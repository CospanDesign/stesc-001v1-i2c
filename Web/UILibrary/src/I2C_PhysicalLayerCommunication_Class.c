...
extern uint8_t i2c_addr_off;
extern CVBS oBusSensor[1];
...
enum I2C_COMM_REG_ADDR {
  ...
  I2C_REG_STATE         = 5,
  I2C_REG_ACK           = 6,
  I2C_REG_PAST_FAULTS   = 7,
  I2C_REG_CURR_FAULTS   = 8,
  I2C_REG_ALIGN         = 9,
  I2C_REG_START         = 10,
  I2C_REG_FAULT_ACK     = 11,
  I2C_REG_VBUS          = 12
};
...
void I2C_HWInit(pI2CParams_t pI2CParams)
{
  ...
  */
  ...
  I2C_StretchClockCmd(pI2CParams->i2c_peripheral, DISABLE);
...
void* I2C_IRQ_Handler(void* this,unsigned char flags, unsigned short rx_data)
{
      ...
      if (rx_data == 1) {
        ...
        switch(i2c_struct->address){
          ...
          case (I2C_REG_RPM):
            ...
            i2c_struct->tx_buffer[i2c_struct->tx_count++] = (uint8_t) (data >> 24) & 0xFF;
            i2c_struct->tx_buffer[i2c_struct->tx_count++] = (uint8_t) (data >> 16) & 0xFF;
            i2c_struct->tx_buffer[i2c_struct->tx_count++] = (uint8_t) (data >> 8) & 0xFF;
            i2c_struct->tx_buffer[i2c_struct->tx_count++] = (uint8_t) (data >> 0) & 0xFF;
          break;
          case (I2C_REG_TORQUE):
          ...
          default:
            //Send back an ERROR
            i2c_struct->tx_buffer[i2c_struct->tx_count++] = 31;
            i2c_struct->tx_buffer[i2c_struct->tx_count++] = 41;
  ...
  switch (i2c_struct->state){
    ...
    case (I2C_STATE_READ_FROM_MASTER):
      ...
      switch(i2c_struct->address){
        ...
        case (I2C_REG_START):
          ...
          i2c_struct->state = I2C_STATE_IDLE;
          break;
        case (I2C_REG_FAULT_ACK):
          MCI_FaultAcknowledged(cmci);
          i2c_struct->state = I2C_STATE_IDLE;          
          break;
        case (I2C_REG_RPM):
          ...
          else if (abs(d32) < i2c_speed_min_valueRPM){
            if (d32 < 0)
    ...
    case (I2C_STATE_WRITE_TO_MASTER):
      if ((flags == I2C_TX_READY) && (i2c_struct->tx_index < i2c_struct->tx_count)){
        I2C_SendData(params->i2c_peripheral, i2c_struct->tx_buffer[i2c_struct->tx_index]);
        i2c_struct->tx_index++;
      }
      break;
